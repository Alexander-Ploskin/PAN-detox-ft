...
def submit(dataset, model, tokenizer, submit_file, max_length = 128):
    batches = []
    bs = 1
    for k, ds in tqdm(dataset.items()): 
        for curr in tqdm(range(len(ds))):
            batches.append([
                {
                    "role": "user",
                    "content": [{"type": "text", "text": ds[curr]['text']},]
                }
            ])
    ...
    for batch in tqdm(batches):
        ...
        with torch.inference_mode():
            ...
            print(batch)
            inputs = tokenizer.apply_chat_template(batch, add_generation_prompt=False, return_dict = True, truncation = True, return_tensors = "pt", padding = True,  max_length = 128).to("cuda")
